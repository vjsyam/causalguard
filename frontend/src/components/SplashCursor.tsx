...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    const glObj = getWebGLContext(canvas);
    if (!glObj) return;
    const { gl, ext } = glObj;
    ...
    function compileShader(type: any, source: any, keywords?: string[]) {
    ...
    function generateColor() {
      if (!config.RAINBOW_MODE) {
        // Subtle cyber flame palette: orange, cyan, amber
        const palette = ['#f97316', '#06b6d4', '#f59e0b', '#fb923c'];
